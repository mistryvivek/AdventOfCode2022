...
def supply_stacks_retain_order(initial_stack, steps):
    ...
    for step in steps:
        step_re = re.findall(r"move\s*(\d+)\s*from\s*(\d+)\s*to\s*(\d+)", step)
        segment = step_re[0][1][int(step_re[0][1]) - int(step_re[0][0]):]
        print(segment)
    return initial_stack

initial_stack = [["Z","N"],["M","C","D"],["P"]]
...
print(supply_stacks_retain_order(initial_stack, steps))
"""
initial_stack = [["W","R","F"],
                 ["T","H","M","C","D","V","W","P"],
                 ["P","M","Z","N","L"],
...
print(supply_stacks(initial_stack, steps))
